...
def DeepZINB(X_train_,y_train,X_test_,y_test,batch_size):
        ...
        pi,disp,meanz=scDeepCluster.extract_parameter(x=[X_train, sf])
        pi_t,disp_t,mean_t=scDeepCluster.extract_parameter(x=[X_test, sf_test])
        mean=np.log1p(meanz)
        parameter={'pi': pi,  'disp' :disp,    'pi_t': pi_t,   'disp_t':disp_t,    'X_train':(0.2*mean+train_raw),   'X_test':test_raw, 'y_train':y_train,   'y_test':y_test}
        K.clear_session()
        return   parameter
